...
gram.df <- function(){
      tokens <- names(uni.sum2)
      bi.tokens <- names(bi.sum2)
      tri.tokens <- names(tri.sum2)
      quad.tokens <- names(quad.sum2)
...
blogs1 <- blogs[5001:15000]
twitter1 <- twitter[5001:15000]
news1 <- news[5001:15000]
...
uni2 <- TermDocumentMatrix(as.Corpus(as.character(sam)), control = list(tokenize = unigramizer, wordLengths=c(1,Inf)))
gc()
bi2 <- TermDocumentMatrix(as.Corpus(as.character(sam)), control = list(tokenize = bigramizer, wordLengths=c(1,Inf)))
gc()
tri2 <- TermDocumentMatrix(as.Corpus(as.character(sam)), control = list(tokenize = trigramizer,wordLengths=c(1,Inf)))
gc()
quad2 <- TermDocumentMatrix(as.Corpus(as.character(sam)), control = list(tokenize = quadgramizer, wordLengths=c(1,Inf)))
gc()

uni.sum2 <- apply(uni2, 1, sum)
bi.sum2 <- apply(bi2, 1, sum)
tri.sum2 <- apply(tri2, 1, sum)
quad.sum2 <- apply(quad2, 1, sum)
...
t.bi <- mutate(t.bi, count = as.numeric(bi.sum2))

t.tri <- select(df.tri, term1, term2, term3)
t.tri <- mutate(t.tri, count = as.numeric(tri.sum2))

t.quad<- select(df.quad, term1, term2, term3, term4)
t.quad <- mutate(t.quad, count = as.numeric(quad.sum2))
