...
gram.df <- function() {
      ...
      bi.toks <<- stri_split_fixed(bi.sum$terms, " ")
      tri.toks <<- stri_split_fixed(tri.sum$terms, " ")
      quad.toks <<- stri_split_fixed(quad.sum$terms, " ")
      
      bi.toks <<- as.data.frame(matrix(unlist(bi.toks), ncol = 2, byrow = TRUE))
      tri.toks <<- as.data.frame(matrix(unlist(tri.toks), ncol = 3, byrow = TRUE))
      quad.toks <<- as.data.frame(matrix(unlist(quad.toks), ncol = 4, byrow = TRUE))
      
      t.bi <<- data.frame(count = bi.sum$sum)
      t.tri <<- data.frame(count = tri.sum$sum)
      t.quad <<- data.frame(count = quad.sum$sum)
      
      t.uni <<- uni.sum
      t.bi$term1 <<- bi.toks$V1
      t.bi$term2 <<- bi.toks$V2
      t.tri$term1 <<- tri.toks$V1
      t.tri$term2 <<- tri.toks$V2
      t.tri$term3 <<- tri.toks$V3
      t.quad$term1 <<- quad.toks$V1
      t.quad$term2 <<- quad.toks$V2
      t.quad$term3 <<- quad.toks$V3
      t.quad$term4 <<- quad.toks$V4
}
